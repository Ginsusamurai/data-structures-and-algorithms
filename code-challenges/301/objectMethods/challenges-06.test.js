...
const totalCharacters = (arr) => {
  ...
  let charList = [];
  arr.forEach(house => {
    if(!charList.includes(house.name) && house.name !== null){
      charList.push(house.name);
    }
    if(!charList.includes(house.spouse) && house.spouse !== null){
      charList.push(house.spouse);
    }
    if(house.children.length > 0 && house.children !== null){
      house.children.forEach( kid => {
        if(!charList.includes(kid)){
          charList.push(kid);
        }
      });
    }
...
const houseSize = (arr) => {
  ...
  arr.forEach(obj => {
    let counter = 0;
    counter = obj.name !== null ? counter + 1 : counter + 0;
    counter = obj.spouse !== null ? counter + 1 : counter + 0;
    counter = obj.children !== null ? counter + obj.children.length : counter + 0;
    // new Object(obj.house, counter);
    sizes.push({house:obj.house, members:counter,});
  });
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach(obj => {
    let counter = 0;
    // console.log(counter);
    if(obj.name !== null && !deceasedSpouses.includes(obj.name)){
      counter += 1;
    }
    ...
    // new Object(obj.house, counter);
    survivors.push({house:obj.house, members:counter,});
  });
