...
const reversedString = (str) => {
  return str.split('').reduce((acc, val, ind, array) => {
    // console.log(acc);
    acc.unshift(val);
    return acc;
...
const countNumberOfChildren = (arr) => {
  return arr.reduce((acc, val, ind, array) => {
    return val.children ? acc + val.children.length : acc + 0;
  },0);
};
...
const calculateAverage = (arr) => {
  let x = arr.reduce((acc, val, ind, array) => {
    acc.count = acc.count + 1;
    acc.sum = acc.sum + val;
    // console.log(acc);
    return acc;
  },{count:0, sum:0,});
  // console.log(x.sum, x.count, x.sum / x.count);
